...
from .models import Game, Player, Stats, Rival
...
class AddStatsForm(forms.ModelForm):
    ...
    player = forms.ModelChoiceField(label="player", queryset=Player.objects.all())
    ...
    class Meta:
        model = Stats
        fields = ('player', 'goals', 'assists', 'intercepts',
                  'dribbles', 'tuckles', 'remark')
            ...
            if form.is_valid_stats():
                ...
                if player.name in statsset:
                    raise forms.ValidationError(
                        "Stats in a set have distinct player")
                else:
                    statsset.append(player.name)


StatsFormSet = inlineformset_factory(
    Game, Stats, form=StatsForm, formset=CustomStatsFormSet, extra=5)
